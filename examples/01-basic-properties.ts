 ...
 * run a test body 100 times with random inputs, and `draw()` on
...
await check("float absolute value is non-negative", async () => {
  const x = await draw(floats(-1000, 1000));
...
await check("text concat length", async () => {
  const s1 = await draw(text(0, 20));
  const s2 = await draw(text(0, 20));
...
await check("division: Euclidean identity", async () => {
  const n = await draw(integers(-100, 100));
  const d = await draw(integers(-20, 20));
