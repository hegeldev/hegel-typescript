 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.36.3) via {@link koffi}.
 *
 ...
 *
 * A few optional C parameters the client never varies are absorbed the same
 * way, hidden from the wrappers below: `hegel_generate_boolean`'s
 * `forced` / `has_forced` (always `false, false` — the client never forces a
 * draw) and `hegel_string_generator_regex`'s `alphabet` (always NULL — no
 * custom alphabet).
 */
