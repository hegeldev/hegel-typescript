...
describe("basic property tests", () => {
  ...
  test("composite generator", () => {
    const pairGen = composite((tc) => {
      const x = tc.draw(integers({ minValue: 0, maxValue: 100 }));
      const y = tc.draw(integers({ minValue: x, maxValue: 100 }));
  ...
  test("assume rejects invalid inputs", () => {
    let count = 0;
    hegel((tc) => {
      const x = tc.draw(integers({ minValue: 0, maxValue: 100 }));
      tc.assume(x > 10);
