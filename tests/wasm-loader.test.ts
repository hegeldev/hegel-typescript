...
describe("loadWasm", () => {
  it("loads ArrayBuffer and precompiled Module inputs", async () => {
    expect((await loadWasm(wasmBytes)).version()).toBe("0.38.1");
    expect((await loadWasm(wasmModule)).version()).toBe("0.38.1");
  });
  it("fetches the supplied URL and prefers streaming", async () => {
    ...
    expect((await loadWasm(url)).version()).toBe("0.38.1");
  ...
  it("clones a consumed response for MIME fallback", async () => {
    ...
    expect((await loadWasm(supplied)).version()).toBe("0.38.1");
    expect(supplied.bodyUsed).toBe(true);
  });
  it("falls back for actual missing/wrong MIME headers", async () => {
    expect((await loadWasm(response("text/plain"))).version()).toBe("0.38.1");
    expect((await loadWasm(new Response(wasmBytes))).version()).toBe("0.38.1");
  });
  it("falls back for an actual parameterized Wasm MIME header", async () => {
    const streaming = vi.spyOn(WebAssembly, "instantiateStreaming");
    const fallback = vi.spyOn(WebAssembly, "instantiate");
    expect((await loadWasm(response("application/wasm; charset=utf-8"))).version()).toBe("0.38.1");
    expect(streaming).toHaveBeenCalledTimes(1);
  ...
  it("uses bytes if streaming is unavailable", async () => {
    ...
    expect((await loadWasm(response())).version()).toBe("0.38.1");
  });
  ...
  it("rejects wrong versions and import shapes", async () => {
    vi.spyOn(WasmEngine.prototype, "version").mockReturnValueOnce("0.38.0");
    await expect(loadWasm(wasmModule)).rejects.toThrow("expected 0.38.1, got 0.38.0");
